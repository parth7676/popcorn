...
import * as  onTheAirActions from './actions';
...
class OnTheAir extends React.Component {
    ...
    componentDidMount() {
        this.props.actions.laodOnTheAirTVShows(API_KEY, this.state.pageIndex);
    }

    loadMore() {
        let totalPages = this.props.onTheAirTVShows.toJS().totalPages;
        let newPageIndex = this.state.pageIndex + 1;
        this.setState({ pageIndex: newPageIndex });
        if (newPageIndex <= totalPages) {
            this.setState({ dataLoading: false });
            this.props.actions.loadOnTheAirTVShows(API_KEY, newPageIndex);
        }
    }
    ...
    render() {
        ...
        let tvShows = this.props.onTheAirTVShows.toJS().onTheAirTVShows && this.props.onTheAirTVShows.toJS().onTheAirTVShows || [];
...
OnTheAir.propTypes = {
    ...
    onTheAirTVShows: PropTypes.object.isRequired,
    apiConfig: PropTypes.object.isRequired,
};

OnTheAir.defaultProps = {
    actions: {},
    onTheAirTVShows: {},
    apiConfig: {},
}
...
const mapStateToProps = state => ({
    onTheAirTVShows: state.get('onTheAirTVShows'),
    apiConfig: state.get('globalState'),
});

function mapDispatchToProps(dispatch) {
    return {
        actions: bindActionCreators(onTheAirActions, dispatch),
    };
}

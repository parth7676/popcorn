...
class Popular extends React.Component {
    ...
    componentDidMount() {
        this.props.actions.loadPopularTVShows(API_KEY, this.state.pageIndex);
    }

    loadMore() {
        let totalPages = this.props.popularTVShows.toJS().totalPages;
        let newPageIndex = this.state.pageIndex + 1;
        this.setState({ pageIndex: newPageIndex });
        if (newPageIndex <= totalPages) {
            this.setState({ dataLoading: false });
            this.props.actions.loadPopularTVShows(API_KEY, newPageIndex);
        }
    }
    ...
    render() {
        ...
        let tvShows = this.props.popularTVShows.toJS().popularTVShows && this.props.popularTVShows.toJS().popularTVShows || [];
...
Popular.propTypes = {
    ...
    popularTVShows: PropTypes.object.isRequired,
...
const mapStateToProps = state => ({
    popularTVShows: state.get('popularTVShows'),
